...
input.addEventListener("keyup", async () => {
  ...
  //HINT: remember to get the specific field in the JSON you want to use
    res.then((suggestionsObject) => {
        let sug = suggestionsObject.suggestions
        ...
        for (let i = 0; i < sug.length; i++) {
            //let result = res[i];
            suggestionList.innerHTML += `<li tabindex="0"><button class="btn">` + sug[i] + `</button></li>`;
        }
